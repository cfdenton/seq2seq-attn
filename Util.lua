...
function Util.logbmm2(res, tensor1, tensor2, t1)
   ...
   if dim2[2] ~= 1 then
      if false and t1.THCUDAMOD then
         res2 = res:view(batch * dim1[2] * dim2[3], 1)
         t1.THCUDAMOD.LogSpace_sum(res2:cdata(), t1:view(batch * dim1[2] * dim2[3], dim2[2]):cdata())
      else
         res2 = res:view(batch, dim1[2], dim2[3], 1)
...
function Util.logbmm(res, resSign, tensor1, tensor2, tensor1Sign, tensor2Sign, t1, t2, t1_start, t2_start)
   -- performs batch matrix multiplication in log space
   local batch = tensor2:size(1)
   local dim2 = tensor2:size()
   local dim1 = tensor1:size()
...
function Util.logadd(result, sign, tensor1, tensor2, tensor1Sign, tensor2Sign, max, ge)
   -- local function zero(i) return (i ~= 0 and i or -1); end
   if false and tensor1.THCUDAMOD then
      result.THCUDAMOD.SignedLogSpace_add(result:cdata(), sign:cdata(),
                                          tensor1:cdata(), tensor2:cdata(),
                                          tensor1Sign:cdata(), tensor2Sign:cdata())
...
function Util.logsum2(sum, sign, tensor, dim, tensorSign, dontchecknan)
   ...
   sum:copy(max):add(sumExp:abs():log())
