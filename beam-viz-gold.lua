...
require 'data.lua'
require 'util.lua'
--require 's2sa.models_grammar_od'
stringx = require('pl.stringx')

cmd = torch.CmdLine()

-- file location
cmd:option('-model', 'cv-en-de-pred/en-de-2-300_final.t7', [[Path to model .t7 file]])
cmd:option('-src_file', '../grammar-hidden/data/nmt-jp-en/jp-dev-char.txt',
	   [[Source sequence to decode (one line per sequence)]])
cmd:option('-targ_file', '../grammar-hidden/data/nmt-jp-en/en-dev-char.txt', [[True target sequence (optional)]])
cmd:option('-output_file', 'pred.txt', [[Path to output the predictions (each line will be the
                                       decoded sequence]])
cmd:option('-src_dict', '../grammar-hidden/data/nmt-jp-en/jp-en-1m-char.src.dict', [[Path to source vocabulary (*.src.dict file)]])
cmd:option('-targ_dict', '../grammar-hidden/data/nmt-jp-en/jp-en-1m-char.targ.dict', [[Path to target vocabulary (*.targ.dict file)]])
...
function generate_beam(model, initial, K, max_sent_l, source, gold)
  ...
  -- Backpointer table.
   ...
   local attn_argmax = {}   -- store attn weights
   local decoder_attn_softmax = {}
   attn_argmax[1] = {}
   decoder_attn_softmax[1] = {}
   ...
   for k = 1, 1 do
      ...
      table.insert(decoder_attn_softmax[1], initial)
...
--	 end	 
	 out_float[k]:add(scores[i-1][k])
	  ...
	  decoder_softmax.output[{{},source_l}]:zero()
       ...
       for k = 1, K do
		...
		if model_opt.attn == 1 then
		   ...
		   decoder_attn_softmax[i][k] = State.advance(decoder_attn_softmax[i-1][prev_k],
							     decoder_softmax.output[prev_k]:clone())
		end		
	        prev_ks[i][k] = prev_k
                next_ys[i][k] = y_i
                scores[i][k] = score
	  ...
	  rnn_state_dec[j]:copy(rnn_state_dec[j]:index(1, prev_ks[i]))
       ...
       end_decoder_attn_softmax = decoder_attn_softmax[i][1]
	  ...
	  for k = 1, K do
		...
		if scores[i][k] > max_score then
		   ...
		   max_decoder_attn_softmax = decoder_attn_softmax[i][k]		   
		end
	     end	     
	  end	  
       end       
   ...
   if opt.score_gold == 1 then
	 ...
	 local out_decoder = model[2]:forward(decoder_input)	 
	 table.insert(gold_attn, decoder_softmax.output[1]:clone())
	 local out = model[3]:forward(out_decoder[#out_decoder]) -- K x vocab_size
	 rnn_state_dec = {} -- to be modified later
	 table.insert(rnn_state_dec, out_decoder[#out_decoder])
	 for j = 1, #out_decoder - 1 do
   ...
   if opt.simple == 1 or end_score > max_score or not found_eos then
      ...
      max_decoder_attn_softmax = end_decoder_attn_softmax      
   end
   print(gold_attn)
   return max_hyp, max_score, max_attn_argmax, gold_score, states[i], scores[i], attn_argmax[i], concat_tensor(max_decoder_attn_softmax), concat_tensor(gold_attn)
end

function concat_tensor(t)
   if t == nil then
      return nil
   end
   local r = torch.Tensor(#t-1, t[2]:size(1))
   for i = 2, #t do
      r[i-1]:copy(t[i])
   end
   return r
...
function main()
   ...
   for line in file:lines() do
	 ...
	 target, target_str = sent2wordidx(gold[sent_id], word2idx_targ, 1)
      ...
      pred, pred_score, attn, gold_score, all_sents, all_scores, all_attn, dec_attn, gold_attn = generate_beam(model,
      ...
      print('PRED ' .. sent_id .. ': ' .. gold[sent_id])
      if gold ~= nil then
	 ...
	 if opt.score_gold == 1 then
      ...
      local attn_prob = pp_tensor(gold_attn, 'attn')	
      print(attn_prob)
