...
--	 end	 
   ...
   return max_hyp, max_score, max_attn_argmax, gold_score, states[i], scores[i], attn_argmax[i], concat_tensor(max_decoder_attn_softmax)
end
...
function get_layer(layer)
   if layer.name ~= nil then
	 ...
	 word_vecs_dec = layer
      ...
      if layer.name:sub(1,7) == 'softmax' then
...
function main()
   ...
   for line in file:lines() do
	 ...
	 target, target_str = sent2wordidx(gold[sent_id], word2idx_targ, 1)
      ...
      pred, pred_score, attn, gold_score, all_sents, all_scores, all_attn, dec_attn = generate_beam(model,
	 ...
	 if opt.score_gold == 1 then
      ...
      local attn_prob = pp_tensor(dec_attn, 'attn')	
      print(attn_prob)
