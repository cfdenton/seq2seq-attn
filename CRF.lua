...
function CRF:updateOutput(input)
   ...
   -- Expands the weights to form an entire lattice. At the start and end nodes,
   -- weights are added up, and weights are zeroed for non-existent nodes at the
   -- start and end of the lattice. (note that log 0 = -inf)
   ...
   self._expandedWeight[{{}, 1, 1}]:copy(torch.sum(self._expandedWeight[{{}, 1}],2):div(self.nstates))
   self._expandedWeight[{{}, n+1, {}, 1}]:copy(torch.sum(self._expandedWeight[{{}, n+1}], 3):div(self.nstates))
...
function CRF:updateGradInput(input, gradOutput)
   ...
   -- dL/d(node) = alpha*beta_hat + beta*alpha_hat at that node
   ...
   self._nodeMarginals:resizeAs(nodes):copy(nodes):add(self._logTables[{2, {2, -2}}]:transpose(1, 2))
...
function CRF:accGradParameters(input, gradOutput, scale)
   ...
   local batches = input:size(1)
   local n = input:size(2)
   local C = self.nstates

   -- backwardGrad and forwardGrad don't include their ending nodes, so we add them back in
   local logGradOutputSign = torch.sign(gradOutput)

   -- dL/dw = alpha*backwardGrad*w + beta*forwardGrad*w + dL/dM*dM/dw at that edge
   ...
   -- do necessary matrix multiplications and add up
